...
import 'package:weather_now/core/helpers/weather_colors.dart';
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<WeatherInfoCubit, WeatherInfoState>(
      ...
      builder: (context, state) {
        ...
        if (state is WeatherInfoSuccess) {
          /// Extract the weather condition text from the API response to determine the weather type.
          final weatherCondition = state.weatherResult.current.condition.text;
          /// Determine the weather type based on the condition text and get the corresponding background gradient.
          final weatherType = resolveWeatherType(weatherCondition);
          /// Determine if it's day or night based on the API response and get the appropriate background gradient.
          final bool isDay = state.weatherResult.current.isDay == 1;
          /// Get the hourly weather data for the current day to display in the hourly weather info section.
          final weatherPerHour =
              state.weatherResult.forecast.forecastday[0].hour;
          /// Get the daily weather data for the upcoming days to display in the daily weather info section.    
          final weatherDays = state.weatherResult.forecast.forecastday;
          /// Get the current humidity and wind speed to display in their respective sections.
          final weatherHumidity = state.weatherResult.current.humidity;
          /// Get the current wind speed to display in the wind speed section.
          final weatherWindspd = state.weatherResult.current.windKph;
          /// Get the background gradient based on the resolved weather type.
          final backgroundGradient = getWeatherBackground(weatherType, isDay);
